...
#
# Another real-world FCP export shape: a sync-clip with NO inner <spine>,
# where the external recorder is nested as a lane=-1 asset-clip INSIDE the
# primary camera asset-clip rather than as a sibling. Before the fix, the
# parser scanned only direct children of <sync-clip> and children of
# <sync-clip>/<spine>, so the lane-attached external audio was never
# collected. With the camera mic muted, the segment fell to is_muted=True
# and got dropped from the timeline-audio plan — the bundled transcriber
# rendered silence for that half of the interview.
