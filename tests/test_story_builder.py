...
class TestStoryBuildMenuPruning:
    ...
    def test_menu_drops_low_score_segments(self, monkeypatch):
        ...
        def _stub_call_ai(prompt, system_prompt="", **_kwargs):
    ...
    def test_fallback_when_all_segments_are_low(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(ai_analysis, '_call_ai', lambda p, s="", **_k: (captured.update(prompt=p), '{"clips": []}')[1])
        monkeypatch.setattr(ai_analysis, 'inject_my_style', lambda p, profile_id=None: p)
...
class TestStoryBuildOrdering:
    ...
    def test_menu_groups_by_score_not_timecode(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda p, s="", **_k: (captured.update(prompt=p, system=s), '{"clips": []}')[1],
        )
        monkeypatch.setattr(ai_analysis, 'inject_my_style', lambda p, profile_id=None: p)
    ...
    def test_prompt_contains_mandatory_reorder_directive(self, monkeypatch):
        ...
        def _stub(prompt, system_prompt="", **_kwargs):
            captured['prompt'] = prompt
    ...
    def test_non_chronological_order_preserved_through_hydration(self, monkeypatch):
        ...
        monkeypatch.setattr(ai_analysis, '_call_ai', lambda p, s="", **_k: fake_response)
        monkeypatch.setattr(ai_analysis, 'inject_my_style', lambda p, profile_id=None: p)
