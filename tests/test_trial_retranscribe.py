"""Retranscribe hardening + share-stays-open (Studio channel).

Adapted from the direct-channel suite (release/direct-core @ 60d1af4,
2026-06-10). The retranscribe tests are channel-shared and ported verbatim:
/retranscribe validates the source BEFORE destroying state, refuses (409)
while a job is live, and clears the previous run's job state.

The share tests are INVERTED relative to the commercial channels: Studio is
the share-enabled line, so /share/<id> must render (NOT 404) and the editor
page must keep its Share entry points. Porting the commercial
test_share_routes_fail_closed as-is would (correctly) fail here.
"""
import json
import os
import sys
...
def test_share_routes_stay_open(client, trial_project):
    """Studio is the share-enabled channel — share routes must NOT 404."""
    pid, _ = trial_project
    # shared_view renders the read-only project experience.
    assert client.get(f'/share/{pid}').status_code == 200
    # Legacy review URL redirects into the shared view.
    resp = client.get(f'/review/{pid}')
    assert resp.status_code in (301, 302)
    assert f'/share/{pid}' in resp.headers.get('Location', '')
    # Share-settings endpoint exists (NOT removed like the commercial channels).
    assert client.get(f'/project/{pid}/share-settings').status_code != 404


def test_share_entry_points_present_in_project_page(client, trial_project):
    """The editor page keeps its Share JS/entry points on Studio."""
    pid, _ = trial_project
    html = client.get(f'/project/{pid}').get_data(as_text=True)
    assert 'copyShareLink' in html
    assert 'generateShareLink' in html
